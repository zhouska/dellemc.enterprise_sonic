...
class L3_interfacesFacts(object):
    ...
    def get_l3_interfaces(self):
        ...
        for l3 in l3_lists:
            ...
            ip = None
            ipv4_anycast_addr = list()
            ipv6_anycast_addr = list()
            if l3.get('openconfig-vlan:routed-vlan'):
                ip = l3['openconfig-vlan:routed-vlan']
                # Handle IPv4 Anycast Address
                if ip.get('openconfig-if-ip:ipv4', None) and ip['openconfig-if-ip:ipv4'].get('openconfig-interfaces-ext:sag-ipv4', None):
                    if ip['openconfig-if-ip:ipv4']['openconfig-interfaces-ext:sag-ipv4'].get('config', None):
                        if ip['openconfig-if-ip:ipv4']['openconfig-interfaces-ext:sag-ipv4']['config'].get('static-anycast-gateway', None):
                            ipv4_anycast_addr = ip['openconfig-if-ip:ipv4']['openconfig-interfaces-ext:sag-ipv4']['config']['static-anycast-gateway']
                # Handle IPv6 Anycast Address
                if ip.get('openconfig-if-ip:ipv6', None) and ip['openconfig-if-ip:ipv6'].get('openconfig-interfaces-ext:sag-ipv6', None):
                    if ip['openconfig-if-ip:ipv6']['openconfig-interfaces-ext:sag-ipv6'].get('config', None):
                        if ip['openconfig-if-ip:ipv6']['openconfig-interfaces-ext:sag-ipv6']['config'].get('static-anycast-gateway', None):
                            ipv6_anycast_addr = ip['openconfig-if-ip:ipv6']['openconfig-interfaces-ext:sag-ipv6']['config']['static-anycast-gateway']
            else:
            ...
            l3_dict['ipv4'] = dict()
            l3_ipv4 = list()
            if ipv4_anycast_addr:
                l3_dict['ipv4']['anycast_addresses'] = ipv4_anycast_addr
            ...
            l3_dict['ipv6'] = dict()
            l3_ipv6 = list()
            if ipv6_anycast_addr:
                l3_dict['ipv6']['anycast_addresses'] = ipv6_anycast_addr
